...
{
	//???????UIFontNode* pFont = UIGraph::GetFont(GetStyle(9)->iFont);
                    ...
                    if(lpCandList)
                    {
                        ...
                        if(!CIME::s_bVerticalCand)
                        {
                          ...
                          //  wszCand[lstrlenW(wszCand) - 1] = L'\0';  // Remove the last space
                            CIME::s_CandList.HoriCand.SetText(wstrCand);
                        }
                    }
                    break;
                }
...
void CUIIMEEditBox::RenderCandidateReadingWindow(float fElapsedTime, bool bReading)
{
    ...
    } else
    {
        ...
        if(bReading)
			...
			UIGraph::CalcTextRect(CIME::s_CandList.HoriCand.GetBuffer(), rc);
    ...
    // Render the elements
	// 候选字词列表背景
    UIGraph::FillRect(rc, clrTextBkColor);
    if((CIME::s_bVerticalCand && !bReading) ||
        (!CIME::s_bHorizontalReading && bReading))
    {
        // Vertical candidate window
        for(UINT i = 0; i < nNumEntries; ++i)
        {
            ...
            if(CIME::s_CandList.dwSelection == i)
            {
                ...
                m_Style.m_crFontColor[1] = clrSelTextColor;
            } else
                m_Style.m_crFontColor[1] = clrTextColor;
    ...
    } else
    {
        ...
        if(bReading)
            ...
            UIGraph::DrawText(CIME::s_CandList.HoriCand.GetBuffer(), m_Style, 1, rc);
...
void CUIIMEEditBox::RenderComposition(float fElapsedTime)
{
    ...
    m_Style.m_crFontColor[1] = TextColor;
    ...
    for(; ;)
    {
        ...
        UIGraph::CalcTextRect(pwszComp, rc/*, nNumCharToDraw*/);
        ...
        if(CIME::GetPrimaryLanguage() == LANG_KOREAN)
        {
            if(m_bCaretOn)
            {
                UIGraph::FillRect(rc, m_CompWinColor);
            }
            else
            {
          ...
          *pcComp != L'\0'; ++pcComp, ++pAttr)
    {
        ...
        {
            m_Style.m_crFontColor[1] = m_CompTargetColor;
            bkColor = m_CompTargetBkColor;
        }
        else
        if(*pAttr == ATTR_TARGET_NOTCONVERTED)
        {
            m_Style.m_crFontColor[1] = m_CompTargetNonColor;
        ...
        UIGraph::FillRect(rcTarget, bkColor);
        UIGraph::DrawText(pcComp, m_Style, 1, rcTarget, false, 1);
    ...
    if(m_bCaretOn)
    {
        ...
        UIGraph::FillRect(rcCaret, m_CompCaretColor);
    }
}

void CUIIMEEditBox::RenderIndicator(float fElapsedTime)
{
    //CUIStyle *pStyle = GetStyle(9);
	m_Style.Blend(CONTROL_STATE_NORMAL, fElapsedTime);

    UIGraph::DrawSprite(m_Style, 1, m_rcIndicator);
    RECT rc = m_rcIndicator;
    m_Style.m_crFontColor[1] = CIME::s_ImeState == CIME::IMEUI_STATE_ON && CIME::s_bEnableImeSystem ? m_IndicatorImeColor : m_IndicatorEngColor;
    RECT rcCalc = { 0, 0, 0, 0 };
    ...
    UIGraph::CalcTextRect(pwszIndicator, rcCalc);
