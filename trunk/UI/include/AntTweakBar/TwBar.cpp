	...
	if(!m_IsMinimized)
	{
		...
		if(!m_IsPopupList)
		{
			...
			if(m_ColTitleShadow!=0)
				Gr->DrawTextObj(m_TitleTextObj, m_PosX+(m_Width-m_TitleWidth)/2+1, m_PosY+1, m_ColTitleShadow, 0);
			Gr->DrawTextObj(m_TitleTextObj, m_PosX+(m_Width-m_TitleWidth)/2, m_PosY, m_ColTitleText, 0);
		...
		// Draw labels
		PERF(Timer.Reset();)
		Gr->DrawTextObj(m_LabelsTextObj, m_PosX+LevelSpace+6, m_PosY+m_VarY0, 0 /*m_ColLabelText*/, 0);
		...
		{
			PERF(Timer.Reset();)
			Gr->DrawTextObj(m_ValuesTextObj, m_PosX+m_VarX1, m_PosY+m_VarY0, 0 /*m_ColValText*/, 0 /*m_ColValBg*/);
...
//				else if(Grp->m_SummaryCallback==CustomTypeSummaryCB && Grp->m_StructValuePtr!=NULL)
		...
		if(m_HighlightedLine>=0 && m_HighlightedLine==m_ShortcutLine && !m_IsPopupList && !m_EditInPlace.m_Active)
		{
			...
			Gr->DrawTextObj(m_ShortcutTextObj, m_PosX+m_Font->m_CharHeight, m_PosY+m_VarY1+1, m_ColShortcutText, 0);
		...
		else if(m_IsHelpBar)
		{
			if(g_TwMgr->m_KeyPressedTextObj && g_TwMgr->m_KeyPressedStr.size()>0)	// Draw key pressed
			{
				...
				PERF(Timer.Reset();)	
				Gr->DrawRect(m_PosX+m_Font->m_CharHeight-2, m_PosY+m_VarY1+1, m_PosX+m_Width-m_Font->m_CharHeight-2, m_PosY+m_VarY1+1+m_Font->m_CharHeight, m_ColShortcutBg);
				Gr->DrawTextObj(g_TwMgr->m_KeyPressedTextObj, m_PosX+m_Font->m_CharHeight, m_PosY+m_VarY1+1, m_ColShortcutText, 0);
			...
			else
			{
				...
				PERF(Timer.Reset();)	
				Gr->DrawRect(m_PosX+m_Font->m_CharHeight-2, m_PosY+m_VarY1+1, m_PosX+m_Width-m_Font->m_CharHeight-2, m_PosY+m_VarY1+1+m_Font->m_CharHeight, m_ColShortcutBg);
				Gr->DrawTextObj(g_TwMgr->m_InfoTextObj, m_PosX+m_Font->m_CharHeight, m_PosY+m_VarY1+1, m_ColInfoText, 0);
	...
	else // minimized
	{
		...
		if(m_HighlightMaximize)
		{
			...
			if(m_ColTitleShadow!=0)
				Gr->DrawTextObj(m_TitleTextObj, m_MinPosX+MinXOffset+m_Font->m_CharHeight/2, m_MinPosY+1+MinYOffset, m_ColTitleShadow, 0);
			Gr->DrawTextObj(m_TitleTextObj, m_MinPosX+MinXOffset+m_Font->m_CharHeight/2, m_MinPosY+MinYOffset, m_ColTitleText, 0);
		}
...
bool CTwBar::MouseButton(ETwMouseButtonID _Button, bool _Pressed, int _X, int _Y)
{
	...
	if(!m_IsMinimized)
	{
		...
		{
			const WCHAR *WebPage = _T("http://www.antisphere.com/Wiki/tools:anttweakbar");
			#if defined ANT_WINDOWS
				ShellExecute(NULL, _T("open"), WebPage, NULL, NULL, SW_SHOWNORMAL);
...
void CTwBar::EditInPlaceDraw()
{
	...
	g_TwMgr->m_Graph->DrawTextObj(m_EditInPlace.m_EditTextObj, m_PosX+m_EditInPlace.m_X, m_PosY+m_EditInPlace.m_Y, ColText, ColBg);
	...
	if(SelWidth>0 && StrSelected.length()>0)
	{
		...
		if (m_EditInPlace.m_CaretPos>m_EditInPlace.m_SelectionStart)
			g_TwMgr->m_Graph->DrawTextObj(m_EditInPlace.m_EditSelTextObj, CaretX-SelWidth, m_PosY+m_EditInPlace.m_Y, m_ColEditSelText, ColSelBg);
		else
			g_TwMgr->m_Graph->DrawTextObj(m_EditInPlace.m_EditSelTextObj, CaretX, m_PosY+m_EditInPlace.m_Y, m_ColEditSelText, ColSelBg);
	}
