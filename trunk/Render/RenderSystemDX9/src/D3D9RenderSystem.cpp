...
CTexture* CD3D9RenderSystem::GetRenderTarget()
{
	...
	if (pRenderTarget)
	{
		D3DCheckHresult( m_pD3D9Device->GetRenderTarget(0 , &pRenderTarget->m_pD3D9Surface) );
	}
...
void CD3D9RenderSystem::SetRenderTarget(CTexture* pRenderTarget)
{
	...
	D3DCheckHresult( m_pD3D9Device->SetRenderTarget(0, pD3D9Surface) );
...
CTexture* CD3D9RenderSystem::GetDepthStencil()
{
	...
	if (pDepthStencil)
	{
		D3DCheckHresult( m_pD3D9Device->GetDepthStencilSurface(&pDepthStencil->m_pD3D9Surface) );
	}
...
void CD3D9RenderSystem::ClearBuffer (bool bZBuffer, bool bTarget, Color32 color)
{
	...
	D3DCheckHresult( m_pD3D9Device->Clear(1, &rect, dwFlags, color.c, 1.0f, 0 ));
}
...
void CD3D9RenderSystem::setWorldMatrix(const Matrix& m)
{
	...
	D3DCheckHresult( m_pD3D9Device->SetTransform(D3DTS_WORLD, (D3DXMATRIX*)&mDx) );
}

void CD3D9RenderSystem::setViewMatrix(const Matrix& m)
{
	Matrix mDx=m;mDx.transpose();
	D3DCheckHresult( m_pD3D9Device->SetTransform(D3DTS_VIEW, (D3DXMATRIX*)&mDx) );
}

void CD3D9RenderSystem::setProjectionMatrix(const Matrix& m)
{
	Matrix mDx=m;mDx.transpose();
	D3DCheckHresult( m_pD3D9Device->SetTransform(D3DTS_PROJECTION, (D3DXMATRIX*)&mDx) );
}
...
void CD3D9RenderSystem::setTextureMatrix(uint8 uTexChannel, TextureTransformFlag flag, const Matrix& m)
{
	...
	D3DCheckHresult( m_pD3D9Device->SetTextureStageState(uTexChannel, D3DTSS_TEXTURETRANSFORMFLAGS, uD3D9Flag) );
	Matrix mDx=m;mDx.transpose();
...
{
	D3DCheckHresult( m_pD3D9Device->GetTransform(D3DTS_WORLD, (D3DXMATRIX*)&m) );
	m.transpose();
}

void CD3D9RenderSystem::getViewMatrix(Matrix& m)const
{
	D3DCheckHresult( m_pD3D9Device->GetTransform(D3DTS_VIEW, (D3DXMATRIX*)&m) );
	m.transpose();
}

void CD3D9RenderSystem::getProjectionMatrix(Matrix& m)const
{
	D3DCheckHresult( m_pD3D9Device->GetTransform(D3DTS_PROJECTION, (D3DXMATRIX*)&m) );
	m.transpose();
}
...
void CD3D9RenderSystem::DrawPrimitive(VertexRenderOperationType PrimitiveType,uint32 StartVertex,uint32 PrimitiveCount)
{
	if (commit())// 检测状态是否正确
	{
		D3DCheckHresult( m_pD3D9Device->DrawPrimitive(VertexRenderOperationTypeForD3D9(PrimitiveType),StartVertex,PrimitiveCount) );
	}
}
...
void CD3D9RenderSystem::DrawIndexedPrimitive(VertexRenderOperationType PrimitiveType,int32 BaseVertexIndex,uint32 MinVertexIndex,uint32 NumVertices,uint32 startIndex,uint32 primCount)
{
	if (commit())// 检测状态是否正确
	{
		D3DCheckHresult( m_pD3D9Device->DrawIndexedPrimitive(VertexRenderOperationTypeForD3D9(PrimitiveType), BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount) );
	}
}
...
void CD3D9RenderSystem::DrawPrimitiveUP(VertexRenderOperationType PrimitiveType,uint32 PrimitiveCount,const void* pVertexStreamZeroData,uint32 VertexStreamZeroStride)
{
	if (commit())// 检测状态是否正确
	{
		D3DCheckHresult( m_pD3D9Device->DrawPrimitiveUP(VertexRenderOperationTypeForD3D9(PrimitiveType), PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride) );
	}
}
...
void CD3D9RenderSystem::DrawIndexedPrimitiveUP(VertexRenderOperationType PrimitiveType,uint32 MinVertexIndex,uint32 NumVertices,uint32 PrimitiveCount,const void* pIndexData,const void* pVertexStreamZeroData,uint32 VertexStreamZeroStride)
{
	if (commit())// 检测状态是否正确
	{
		D3DCheckHresult( m_pD3D9Device->DrawIndexedPrimitiveUP(VertexRenderOperationTypeForD3D9(PrimitiveType), MinVertexIndex, NumVertices, PrimitiveCount, pIndexData, D3DFMT_INDEX16, pVertexStreamZeroData, VertexStreamZeroStride) );
	}
}
...
void CD3D9RenderSystem::drawIndexedSubset(const IndexedSubset& subset)
{
	if (commit())// 检测状态是否正确
	{
		D3DCheckHresult( m_pD3D9Device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, subset.vbase, subset.vstart, subset.vcount, subset.istart, subset.icount/3) );
	}
}
...
void CD3D9RenderSystem::LightEnable(uint32 Index, bool bEnable)
{
	...
	D3DCheckHresult( m_pD3D9Device->LightEnable(Index, bEnable) );
}
...
bool CD3D9RenderSystem::commitRenderState()
{
	...
	{
		DWORD dwOldValue = 0;
		D3DCheckHresult( m_pD3D9Device->GetRenderState(static_cast<D3DRENDERSTATETYPE>(it->first), &dwOldValue) );
		if (dwOldValue != it->second)
		{
			D3DCheckHresult( m_pD3D9Device->SetRenderState(static_cast<D3DRENDERSTATETYPE>(it->first), it->second) );
		}	
	}
	m_mapChangeRenderState.clear();
	return true;
...
bool CD3D9RenderSystem::commitSamplerstate()
{
	for (std::map<uint32,uint32>::iterator it=m_mapChangeSamplerState.begin(); it!=m_mapChangeSamplerState.end(); ++it)
	{
		...
		DWORD dwOldValue	= 0;
		D3DCheckHresult( m_pD3D9Device->GetSamplerState(nSamplerID,(D3DSAMPLERSTATETYPE)nTypeID, &dwOldValue) );
		if (dwOldValue != it->second)
		{
			D3DCheckHresult( m_pD3D9Device->SetSamplerState(nSamplerID,(D3DSAMPLERSTATETYPE)nTypeID, it->second) );
		}	
	}
	m_mapChangeSamplerState.clear();
	return true;
...
bool CD3D9RenderSystem::commitTextureStageState()
{
	for (std::map<uint32,uint32>::iterator it=m_mapChangeTextureStage.begin(); it!=m_mapChangeTextureStage.end(); ++it)
	{
		...
		DWORD dwOldValue	= 0;
		D3DCheckHresult( m_pD3D9Device->GetTextureStageState(nStageID, (D3DTEXTURESTAGESTATETYPE)nTypeID, &dwOldValue) );
		if (dwOldValue != it->second)
		{
			D3DCheckHresult( m_pD3D9Device->SetTextureStageState(nStageID, (D3DTEXTURESTAGESTATETYPE)nTypeID, it->second) );
		}
	}
	m_mapChangeTextureStage.clear();
	return true;
...
bool CD3D9RenderSystem::commitTexture()
{
	for (std::map<uint32,IDirect3DTexture9*>::iterator it=m_mapChangeTexture.begin(); it!=m_mapChangeTexture.end(); ++it)
	{
		...
		D3DCheckHresult( m_pD3D9Device->GetTexture(it->first,(IDirect3DBaseTexture9**) &pOldD3D9Texture) );
		if (pOldD3D9Texture != it->second)
		{
			D3DCheckHresult( m_pD3D9Device->SetTexture(it->first, it->second) );
		}	
...
bool CD3D9RenderSystem::commitStreamSource()
{
	for (std::map<uint32,D3D9StreamSource>::iterator it=m_mapChangeStreamSource.begin(); it!=m_mapChangeStreamSource.end(); ++it)
	{
		...
		D3DCheckHresult( m_pD3D9Device->GetStreamSource(it->first,&d3D9StreamSource.pStreamData,&d3D9StreamSource.uOffsetInBytes,&d3D9StreamSource.uStride) );
		if (d3D9StreamSource != it->second)
		{
			D3DCheckHresult( m_pD3D9Device->SetStreamSource(it->first,it->second.pStreamData,it->second.uOffsetInBytes,it->second.uStride) );
		}
...
bool CD3D9RenderSystem::commitOther()
{
	...
	{
		DWORD dwOldFVF = 0;
		D3DCheckHresult( m_pD3D9Device->GetFVF(&dwOldFVF) );
		if (m_uChangeFVF != dwOldFVF)
		{
			D3DCheckHresult( m_pD3D9Device->SetFVF(m_uChangeFVF) );
		}
		m_uChangeFVF = 0;
	}
	...
	if (m_pChangeIB)
	{
		...
		D3DCheckHresult( m_pD3D9Device->GetIndices(&pOldIB) );
		if (pOldIB!=m_pChangeIB)
		{
			D3DCheckHresult( m_pD3D9Device->SetIndices(m_pChangeIB) );
		}
		S_REL(pOldIB);
		m_pChangeIB = NULL;
	}
