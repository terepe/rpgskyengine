...
CDataPlugBase* CDataPlugsMgr::getPlugByExtension(const std::string& strExt)
{
	for (size_t i=0;i<m_setPlugObj.size();++i)
	{
		if (m_setPlugObj[i].pObj->GetFormat()==strExt)
		{
...
bool CDataPlugsMgr::createPlugFromPath(const std::string& strPath,const std::string& strFun)
{
	...
	{
		createPlug(strPath + wfd.cFileName,strFun);
...
bool CDataPlugsMgr::createPlug(const std::string& strFilename,const std::string& strFun)
{
	bool brt = false;
	...
	if (stPs.hIns)
	{
		PFN_DATA_Plug_CreateObject pFunc = (PFN_DATA_Plug_CreateObject)GetProcAddress(stPs.hIns, strFun.c_str());
		if (pFunc)
		{
